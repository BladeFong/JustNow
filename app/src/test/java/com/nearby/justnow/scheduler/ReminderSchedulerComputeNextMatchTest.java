...
public class ReminderSchedulerComputeNextMatchTest {
    ...
    @Test
    public void typeMonthly_dayNotYetPassed_returnsThisMonth() {
        Calendar cal = Calendar.getInstance();
        cal.set(2026, Calendar.JUNE, 10, 8, 0, 0);
        cal.set(Calendar.MILLISECOND, 0);

        TaskScheduleEntity schedule = new TaskScheduleEntity();
        schedule.scheduleType = TaskScheduleEntity.TYPE_MONTHLY;
        schedule.scheduleValue = 15;
        schedule.scheduledTime = 600; // 10:00
    ...
    @Test
    public void typeMonthly_dayPassed_returnsNextMonth() {
        Calendar cal = Calendar.getInstance();
    ...
    @Test
    public void typeMonthly_day31InFeb_clampsTo28() {
        Calendar cal = Calendar.getInstance();
        cal.set(2026, Calendar.FEBRUARY, 1, 8, 0, 0);
        cal.set(Calendar.MILLISECOND, 0);

        TaskScheduleEntity schedule = new TaskScheduleEntity();
        schedule.scheduleType = TaskScheduleEntity.TYPE_MONTHLY;
        schedule.scheduleValue = 31;
        schedule.scheduledTime = 600; // 10:00

        long result = ReminderScheduler.computeNextMatch(schedule, cal.getTimeInMillis());

        // 2026 年 2 月只有 28 天，应 clamp 到 28
    ...
    @Test
    public void typeMonthly_invalidDay_zero_returnsZero() {
        Calendar cal = Calendar.getInstance();
        cal.set(2026, Calendar.JUNE, 10, 8, 0, 0);
        cal.set(Calendar.MILLISECOND, 0);
    ...
    @Test
    public void typeMonthly_invalidDay_32_returnsZero() {
        Calendar cal = Calendar.getInstance();
        cal.set(2026, Calendar.JUNE, 10, 8, 0, 0);
        cal.set(Calendar.MILLISECOND, 0);
