...
### 2.3 左右侧栏主面板比例自适应
为了在有限的屏幕宽度下给任务列表留出合适空间，我们需要根据设备及旋转方向动态调整左侧栏（时间线）与右侧栏（任务列表）的宽度权重比（Weight）：
1. **手机端（默认）**（`res/values/dimens.xml`）：
   - 左侧栏权重：`1.0`
   - 右侧栏权重：`1.618`
2. **平板竖屏**（`res/values-sw600dp/dimens.xml`）：
   - 左侧栏权重：`1.0`
...
|---|---|---|---|
| `ic_activity_blocks` | 玩具 | 城堡积木堆叠（红尖顶、黄圆柱、蓝绿底基） | 自由玩耍、乐高搭建 |
| `ic_activity_book` | 阅读 | 双页展开绘本与挂坠丝带书签 | 听故事、看绘本、课外阅读 |
| `ic_activity_palette` | 美术 | 奶黄底调色盘与四色颜料、跨置木笔杆画笔 | 简笔画、涂色、艺术手工 |
| `ic_activity_music` | 音乐 | 双连紫色与粉紫拼接的八分音符 | 唱歌、跳舞、听儿歌、弹琴 |
| `ic_activity_ball` | 运动 | 红黄蓝三色条纹相间的充气沙滩球 | 户外拍球、跳绳、体育锻炼 |
| `ic_activity_game_puzzle` | 益智 | 绿色与黄色两个拼合在一起的拼图块 | 拼图、智力桌游、益智游戏、走迷宫 |
| `ic_activity_craft` | 手工 | 红黄双色剪刀与斜跨蓝色折纸卡纸 | 创意折纸、手工捏泥、简易DIY |
| `ic_activity_animation` | 屏幕 | 珊瑚橙色卡通天线电视机与双色旋钮 | 看动画片、英语绘本视频、屏幕时间 |
| `ic_activity_study` | 学习 | 蓝色螺旋线圈笔记本与红黄笔身铅笔 | 寒暑假书面作业、练字、口算练习 |
| `ic_activity_chores` | 家务 | 蓝色垃圾铲搭配黄色刷毛及木杆扫帚 | 整理玩具箱、扫地擦桌扫地、给花草浇水 |
...
* **内置图标位置**：在彩色条右侧，新增一个 `ImageView` 作为图标容器：
  * **尺寸升级**：为了呈现更清晰精美的卡通图标细节，并与右侧的两行文本高度对齐（占满两行高度空间），图标宽高设为 **`40dp x 40dp`**（通过 `@dimen/task_icon_size` 统一管理）。
...
2. **界面排布**：
   为了使输入流更加连贯，选择图标卡片 `card_icon_selector` 放置于**任务内容卡片（`card_markdown`）下方、标签输入卡片（`card_tag`）上方**的黄金分割位置。
   * **平板横屏**：由于横向空间极其充裕，10 个图标呈 **1 行 10 列** 水平排列，无需水平滚动，完全平铺。
   * **平板竖屏**：呈 **2 行 5 列** 的网格排列，保证不挤压且对称美观。
3. **用户已添加常用标签自适应展示**：
   用于展示用户添加过的常用标签列表 `cg_existing_tags` 具备屏幕方向自适应排版：
   * **常规/竖屏下**：保留固定两行展示高度（`88dp`），且 `singleLine="false"`，便于平铺多行快速选择。
   * **横屏下**（通过 `values-land` 资源覆盖）：自动收窄高度为单行高（`44dp`），且 `singleLine="true"`，通过横向滚动（或单行滑移）展示以节约宝贵的垂直空间。
4. **选择与取消交互逻辑**：
...
5. **专注时长选项自适应排版**：
    * **手机端（默认）**：保持双行排列，第一行 3 个选项，第二行 2 个选项 + 1 个占位 View。
    * **平板端（`is_tablet = true`）**：由于横向空间非常充足，所有的专注时长选项（5个）一律排布在同一单行内，以节省垂直空间。
6. **数据存盘**：
